...
public class PotatoCannonBehavior implements IBowBehavior {
	...
	public int shootArrow(BowUseContext user, float power, ItemStack stack, InteractionHand hand) {
		if (!(stack.getItem() instanceof PotatoCannonItem item)) return 20;
		var e = user.user();
		var level = e.level();
	...
	public static Vec3 getGunBarrelVec(LivingEntity player, boolean mainHand, Vec3 rel) {
	...
	public static void sendPackets(LivingEntity player, Function<Boolean, ? extends ShootGadgetPacket> factory) {
